...
//
// The thread and post are written as two separate documents (Firestore does not
// support cross-collection transactions in the same batch easily without a
// transaction); the post is patched with the threadId immediately after both
// docs are created, matching the flow used by the frontend service.
func CreateThread(ctx context.Context, input CreateForumThreadInput) (*ForumThread, error) {
	...
	if _, err := postRef.Update(ctx, []firestore.Update{
		...
		// Non-fatal: the thread exists; the frontend can handle a post without threadId
		// being present, but log so ops can fix it.
...
// buildSlug converts a title to a URL-friendly slug (max 80 chars), mirroring
// the logic used by the frontend CreateThread function.
