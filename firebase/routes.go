...
func BuildTeamRosterRoute(league string, teamID uint) string {
	if league == "cfb" {
		return fmt.Sprintf("/%s/roster/%d", league, teamID)
	}
...
// BuildTeamPracticeSquadRoute builds the route pointing to a team's practice squad page.
func BuildTeamPracticeSquadRoute(league string, teamID uint) string {
	return fmt.Sprintf("/%s/practicesquad", league)
}
...
// Example: "injury:cfb:season12:game881:player193"
